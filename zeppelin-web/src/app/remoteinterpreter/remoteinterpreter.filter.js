...
function jobManagerFilter() {
  function filterContext(jobItems, filterConfig) {
    const filterValueInterpreter = filterConfig.filterValueInterpreter;
    const filterValueNotebookName = filterConfig.filterValueNotebookName;
    ...
    if (filterValueNotebookName !== '') {
      filterItems = _.filter(filterItems, function(jobItem) {
        const lowerFilterValue = filterValueNotebookName.toLocaleLowerCase();
